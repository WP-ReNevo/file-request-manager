/**
 * "Editorial" layout: borderless, numbered requested-file entries, a big
 * headline, contact fields as an inline row instead of a boxed panel — a
 * genuinely different composition, not a recolor of Stacked.
 */
...
export default function EditorialLayout({
	...
	const requestedFilesBlock = (
		<div
			className="frm-requested-files frm-requested-files--editorial"
			key="requested-files"
		>
	...
	if (!isLive) {
		return <div className="frm-frontend-app frm-editorial">{content}</div>;
	}

	return (
		<form
			className="frm-frontend-app frm-editorial"
			onSubmit={onSubmit}
			noValidate
		>
			{content}
